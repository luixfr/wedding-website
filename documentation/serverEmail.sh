...
EMAIL="jmoreno@velansolutions.com,r.geethaprema@velansolutions.com,vharikrishna@velansolutions.com"
...
then
  ...
  sleep 15
  printf "$MAILBODY $ACODE \n\n $SITE \n $IP_ADDRESS \n $USER_NAME \n $CLOUD" | mail -s "${MAILSUBJECT}" -a 'From:VJ at WNWA<support@weneverwalkalone.org>' "${EMAIL}"
fi